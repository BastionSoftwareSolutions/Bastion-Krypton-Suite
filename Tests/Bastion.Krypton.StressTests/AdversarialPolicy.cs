...
{
    /// <summary>
    /// The adversarial suite's explicit skip-list (spec §6.3 rule inherited from §6.2: every
    ...
    public static class AdversarialPolicy
    {
        ...
        private static readonly SkipRule[] Skips =
        {
            ...
            // takes ~90 s (bounded — it completes; no hang), which crosses the per-control
            // watchdog on net46's slower GDI path. FIXME: OPEN in BREAKAGE-LOG Phase 5c A8 —
            ...
            new SkipRule("KryptonToolkitPoweredByControl", "size",
                "Extreme sizes are laid out correctly but pathologically slowly (~90 s bounded framework wrap-measure at int.MaxValue) — exceeds the net46 watchdog. FIXME: OPEN, BREAKAGE-LOG Phase 5c A8."),
            ...
            // design — already covered by the graceful-rejection rule — but Size/Font attacks
