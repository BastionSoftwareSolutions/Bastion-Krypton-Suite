...
// =====================================================================================
// Office 2021/2024 Black theme (Bastion Phase 3 fidelity pass, spec §4.3).
// Colours come from PaletteOffice2021Black_BaseScheme (provenance documented there:
// docs\themes\office2021-colours.md "Black variant" + [K] powerpoint-black.jpg;
// expanded-ribbon interiors are DERIVED from the 2019 Black donor −23/channel —
// table open item 1, awaiting Chris's live screenshots). This class supplies the
// #0A0A0A tab row and the flat File tab (File renders like the other tabs — white
// text, no accent block), plus light-glyph caption buttons, the dark check-box
// strip and the base KryptonColorTable365 for the dark surfaces (mirrors
// PaletteOffice2019Black). Derives from PaletteMicrosoft365Base and reuses
...
/// <summary>
/// Provides the Office 2021 Black appearance (Bastion).
/// </summary>
public class PaletteOffice2021Black : PaletteMicrosoft365Base
{
    #region Static Fields

    #region Ribbon Specific Colors

    // [T] office2021-colours.md: title bar / tab row #0A0A0A ([K] rects 250,8 / 300,70 — uniform).
    private static readonly Color _tabRowBackgroundColor = Color.FromArgb(10, 10, 10);

    // [D] File tab hover fill = toolbar-strip tone #1F1F1F (hover open item 1).
    private static readonly Color _ribbonAppButtonDarkColor = Color.FromArgb(31, 31, 31);

    // [T] File tab fill = tab row #0A0A0A (File renders like the other tabs in [K]).
    ...
    /// <summary>
    /// Initialize a new instance of the PaletteOffice2021Black class.
    /// </summary>
    public PaletteOffice2021Black() : base(
        new PaletteOffice2021Black_BaseScheme(),
        _checkBoxList,
        _galleryButtonList,
        _radioButtonArray)
    {
