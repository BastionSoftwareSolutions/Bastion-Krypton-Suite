...
| 3 | Software.Updater | `Sharp Update\**` (17 .cs: SharpUpdater, SharpUpdateXml, SharpUpdateLocalAppInfo, forms, translations) | [henryxrl/SharpUpdate](https://github.com/henryxrl/SharpUpdate) (Henry Xu), itself "modified from AutoUpdater by BetterCoder on Youtube" per its README | **None declared.** The upstream repository has no LICENSE file → default all-rights-reserved copyright; no explicit redistribution permission exists | **Stripped/none** | **Documented** in module `THIRD-PARTY-NOTICES.md` and **escalated** (§3.2) — attribution alone cannot cure a missing licence |
...
| 6b | Drawing.Utilities | `DrawGlowShadow` fragment inside `ImageBoxExtended.cs` | CodeProject gGlowBox (SSDiver2112), inherited via Cyotek ImageBox | **CPOL (probable)** | Comment credit only | **Not fixed — escalated** (§3.1). Flagged in the module notice as "do not ship before review" |
...
No `.cs` file was modified; the build surface of every change is metadata-only. The full solution was rebuilt after the changes (`Krypton Toolkit Suite Extended 2022 - VS2022.sln`, Release, `/restore`, MSBuild 18): **0 errors** (Themes remains unscheduled by design; warnings are the pre-existing baseline).

---

## 6. Outstanding items

1. Chris's decision on §3.1 (gGlowBox fragment; Toggle.Switch module; §1.1 fragment policy).
2. Chris's decision on §3.2 (SharpUpdate — recommend removal; AdvancedWizard — recommend courtesy licence request).
